...
async def get_home_dashboard(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        logger.error("No se encontrÃ³ el archivo de noticias del portafolio para usuario %s: %s", current_user.user_id, exc)
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except Exception as exc:  # pragma: no cover - errores inesperados
        logger.exception("Error al obtener datos para la secciÃ³n de inicio del usuario %s", current_user.user_id)
